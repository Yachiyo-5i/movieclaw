...
def test_library_help_uses_explicit_identification_and_preview_names(run_mclaw) -> None:
    """模型从 help 探索时能直接看懂动作，且两个预览命令都不会被路径覆盖。"""
    identification = run_mclaw("library", "identification", "--help")
    assert identification.returncode == 0, identification.stderr
    assert "assign-file-to-title" in identification.stdout
    assert "assign-files-to-title" in identification.stdout
    assert "identify-file" not in identification.stdout

    items = run_mclaw("library", "items", "--help")
    assert items.returncode == 0, items.stderr
    assert "preview-reidentification" in items.stdout
    assert "preview-transfer" in items.stdout
...
def test_usage_error_exits_2(run_mclaw) -> None:
    result = run_mclaw("subscriptions", "get")  # 缺少必填的 subscription_id
    assert result.returncode == 2


def test_unknown_command_exits_2(run_mclaw) -> None:
    result = run_mclaw("subscriptions", "nonexistent")
    assert result.returncode == 2


def test_removed_legacy_root_commands_stay_removed(run_mclaw) -> None:
    """公开命令树只有 subscriptions/library，不重新注册 sub/lib 双轨入口。"""
    for command in ("sub", "lib"):
        result = run_mclaw(command, "--help")
        assert result.returncode == 2
...
def test_unreachable_server_exits_4(run_mclaw) -> None:
    result = run_mclaw(
        "--server", "http://127.0.0.1:9", "--timeout", "3", "subscriptions", "list"
    )
...
def test_unauthenticated_exits_3_with_login_hint(run_mclaw, live_server) -> None:
    result = run_mclaw("--server", live_server, "subscriptions", "list")
...
def test_server_url_without_scheme_exits_4_with_hint(run_mclaw) -> None:
    ...
    result = run_mclaw(
        "--server", "127.0.0.1:9", "--timeout", "3", "subscriptions", "list"
    )
    assert result.returncode == 4
