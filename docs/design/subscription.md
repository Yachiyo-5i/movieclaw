...
- **电影的上映感知调度**（`movie_schedule`）：TMDB 档期通常是院线上映日，
  未上映/刚上映时大概率没有正规资源，订阅即搜只会白搜。已定档的电影首搜排在
  `上映日 + MOVIE_RELEASE_GRACE（7d）`；明确未上映且未定档（status 非 Released）
  为 `NULL` 等定档回填；status 未知/已上映的老片维持订阅即搜。期间资源到来靠
  被动匹配兜住；上映日**不写进工单 `air_date`**（covered_units 会把它当发布时间
  物理上限，误杀早于档期流出的资源）。用户「立即搜索」对电影是强制语义——未上映
  也搜一次，搜完由退避地板恢复原定档期。
...
worker 就是一条查询：
`SELECT … WHERE in_scope=true AND status='wanted' AND next_search_at <= now ORDER BY priority LIMIT n`。
...
### 2.5 已投递死种换源（与缺口搜索分开调度）

`wanted_item` 的正常搜索只回答“还缺什么”；已投递任务的救援由
`subscription_download_attempt` 保存完成字节心跳、累计网络下载字节与候选历史，不能把工单清空后
重新走缺口队列，否则旧源、新源试用和安全清理三种现实无法同时表达。

- 每 5 分钟读取下载器的 `completed_bytes`。暂停、排队、校验不计时；全部下载器
  不可达只表示状态未知；可达但连续三次查无任务才确认缺失。
- 连续 15 分钟完成字节不增长：任务中心提示“立即换种”；连续 30 分钟：自动执行
  真实跨站搜索。这里不反复扫本地索引；但站点同步新进入索引的种子可以立即抢跑。
- 候选必须继续通过原规则组，且分辨率、Remux、片源等级、HDR/DV 不低于旧源；
  已尝试过的站点种子与实际 infohash 永久排除。
- 新源先以 `trial` 并行投递，累计产生至少 1 MiB **网络下载字节**后才把
  `wanted_item.info_hash` 切到新源；本地校验、复用旧文件产生的完成块不算证明。
  提交前已经存在且已完成的任务可直接接管。此前旧源始终保留。
- 搜索成功但无同品质候选时，旧源继续等待，专用退避为
  `1h → 3h → 12h → 24h`（之后每天一次）；全站技术失败固定 15 分钟短重试，
  不推进退避档位。
- 晋升后只自动清理由 MovieClaw 首次创建且明确无 H&R 的旧任务。新旧文件路径重叠
  时只删任务并留数据；路径确认不重叠时才删任务与数据；所有权、H&R 或路径证据
  不足时保留旧源，并在任务中心说明原因。工单切换先原子落成 `cleanup_pending`，
外部删除被重启或网络故障打断时由后续巡检幂等续做。

取消季订阅时，`wanted_item.status` 与 `info_hash` 仍保留已经发生的投递事实，
只把 `in_scope` 设为 false；没有入域目标的下载尝试同步进入 `cancelled`，停止观察、
提醒与换源。MovieClaw 不替用户删除下载器任务：任务仍存在时在任务中心按外部任务
展示，用户从下载器删除后不再合成“缺失订阅任务”。重新勾选该季则恢复原工单和主源
尝试，避免把同一内容再次投递。所有网络提交在落尝试台账前都必须重检范围，覆盖取消
订阅与提交下载器并发发生的窗口。
