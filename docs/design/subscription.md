...
NULL 视为未知按规则组配置的保守/宽松策略处理）、**字幕语言与音轨语言**
（BCP 47 前缀匹配，要求 "zh" 命中 zh/zh-Hans/zh-Hant；声明式语义——资源未
声明该轴而规则有要求即拒绝，识别来自 torrent-ner v2 的 SUBTITLE/AUDIO span +
lang_decl 微解析，见 subtitle-audio-ner.md。随 v2 模型镜像基线发布，发布日志
须提醒用户升级镜像后音轨维度方可生效）。
