...
### 4.1 结论：快照字段按"可实测性"分层取值，实测能覆盖的一律以实测为准

种子名只在抓取决策那一刻不得不用（候选没下载之前无法 ffprobe）；文件一旦
落地，**能实测的维度一切以 ffprobe 为准**，名称信息只保留在实测测不出的
"出处"维度上，且要过实测合理性校验。**种子名骗得到一次抓取，骗不到台账。**
...
| `media_source` / `remux` | 名称解析（`attempt.quality` 或文件名 enrich），**经码率合理性校验** | 出处不是物理属性，测不出来；但"标称 Remux 而实测码率远低于 Remux 下限"可以证伪 |
| `release_group` | 名称解析 | 仅展示 |
| `bit_rate` | ffprobe 实测 | 合理性校验的证据，随快照留存供详情页解释 |

**码率合理性校验**（matcher 内表驱动 + 单测，阈值刻意从宽只抓明显造假）：
名称声称 T4/T5（蓝光/Remux）而实测码率低于该分辨率档的保守下限 → 该声称
不可信，`media_source`/`remux` 记为**未知**（三态铁律：证伪之后是未知，
不是自作主张降到某一档）。片源未知的单元按 §2.4 部分可比规则继续工作。

### 4.2 为什么不能只信种子名（设计修正记录）

早先方案曾考虑"快照直接取投递时的 `attempt.quality`（种子名解析）、比较
永不二次解析"来防循环。这个方案有一个不可接受的暴露面：**种子名会撒谎**——
标称 Remux 实为二压时，名称基线把单元记到 T5，洗版从此永久停止，用户以为
库里是 Remux。谎言骗到的不是一次下载，是永久的台账。因此基线必须以实测
为锚，防循环改由 §4.3 的机制承担。
...
2. **证伪排除**：实测验证失败的候选记入该单元的排除清单（复用换源"已试过
   ...
   ——arr 缺的正是这一层；
3. **抓取前体积预检**（§5）：把大多数谎言挡在下载发生之前，证伪排除只是
   兜底而非常态。
...
候选侧**体积预检**（下载前唯一可用的证据是体积，用它挡住大多数标称造假）：
候选声称的档位对应一张每集均摊体积的保守下限表（如标称 1080p Remux 但每集
均摊 < 4GB → 明显不实）。低于下限 → 拒绝码 `upgrade_implausible`，按
MATCH_REJECTED 既有去重规则记录（这是用户应当知道的资源质量问题，且天然
低频）。阈值只抓离谱造假，从宽设定，实现期用真实站点数据定标（§11）。
...
新文件来自洗版 attempt（purpose="upgrade"）：
  ...
  否则 → 证伪：
    ...
    - 写 UPGRADE_VERIFY_FAILED 活动
      （"S01E03 洗版候选证伪：标称 Remux，实测码率 9.2 Mbps，已排除该资源"）
...
体积预检（§5）已把大多数造假挡在下载前，证伪路径是低频兜底；它的存在保证
了"抓取判定与导入判定不一致"在结构上**收敛**（同一候选最多付一次代价），
而不是像 arr 一样循环。
...
| Phase | 内容 | 验证 |
|---|---|---|
| 1 | matcher 纯函数：片源档映射表、`upgrade_rank`、`compare_upgrade`、`QualitySnapshot` 归一（probe 值域映射 + 文件名 enrich 路径）、码率合理性校验表、体积预检下限表 | 表驱动单测：真实种子名/文件名/probe 样本集，覆盖未知维度、部分可比、造假样本、整季包、档位边界 |
...
| 4 | 决策接线：`load_match_context` 洗版单元、判定链（排除清单/体积预检/`compare_upgrade`）、整季包逐单元判定、调度排期与退避 | 端到端 dry-run（`SUBSCRIPTION_DISPATCH_DRY_RUN`）：造一个 WEB-DL 已入库的订阅，投放 Remux 种子进缓存，观察判定与活动 |
...
- [ ] 码率合理性校验与体积预检的具体阈值：先收集主要站点各档位的真实
      码率/体积分布再定标，原则是**只抓离谱造假、宁放过不错杀**（动画、
      短片等码率天然偏低的内容必须落在安全区内）；
