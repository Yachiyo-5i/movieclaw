...
- 下载目录与库文件是同一物理文件的部署（inplace 模式）：旧库文件**不进
  回收站**。已实施判据：**`st_nlink == 1`（唯一硬链接）的文件绝不改名**——
  硬链入库的文件（nlink ≥ 2）改名库内目录项不影响下载器侧做种；唯一副本
  则可能正是做种原始文件，一律保留原位、台账行同步保留、UPGRADED 活动
  如实附注，下载器侧任务交给上面的证据链清理，库内残留由用户在库详情
  删除（联动自动删除见 §11 开放问题）。
...
- [x] inplace 模式旧库文件的识别判据：已实施为 `st_nlink == 1`（唯一硬
      链接绝不改名，见 §7.2）；
- [ ] 唯一副本旧库文件在旧 attempt 清理完成后的**联动删除**（当前保留原位
      交用户手动处理；做种达标信号取下载器 seed goal 还是站点 H&R 规则，
      倾向前者）；
