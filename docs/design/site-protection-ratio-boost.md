...
数据源是本地种子索引 `site_torrent`。**开启刷流的站点，索引同步钉在系统
最快节奏**：自适应间隔在成功路径上固定回 5 分钟下限、不再随冷清放疏到
小时级（失败退避照常尊重——对挂掉的站高频重试没有意义；关闭刷流后间隔
按冷热自然漂回）；开启刷流的瞬间游标立即到期，≤2 分钟内完成首刷。免费
新种的发现延迟因此收敛到 5 分钟级——抢免费种拼的就是发现速度。
